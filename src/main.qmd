...
# NOW!
# TODO: make the yield based on LDs only
...
df_summary = df_summary |> 
  mutate(
    ...
    pf_specs  = case_when(
      bi_status == "pre" ~ if_all(c(pf_Ith:pf_KINK)),
      bi_status == "post" ~ if_all(pf_Ith:pf_SMSR, \(x) if_else(is.na(x), FALSE, x))
    )
  )
...
df_summary = df_summary |> 
  ...
  mutate(
    ...
    pf_final = case_when(
      bi_status == "post" ~ if_else(is.na(pf_specs), FALSE, pf_specs) & pf_burn,
      .default = NA
    )
  )
...
df_yield_oe =   df_summary |> 
    summarize(
      ...
      pf_oe_spec = all(
        if_else(is.na(pf_specs), FALSE, pf_specs)
      ),
...
df_yield_oe |> 
  summarize(
    ...
    across(
      .cols = starts_with("pf_"),
      .fns = list(
        fails = \(x) sum(!x, na.rm = FALSE),
        passes = \(x) sum(x, na.rm = FALSE),
        yield = \(x) sum(x, na.rm = FALSE)/n()
      )
    )
  ) |> 
  pivot_longer(
