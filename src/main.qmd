...
df_summary = left_join(
  ...
  by = join_by(bi_status, fc_id, ch, dut_id, temperature),
  suffix = c(".liv", ".osa")
)
...
```{r}
#| label: performance-pf-judgement

df_summary = df_summary |> 
  mutate(
    pf_Ith    = if_else(Ith1d <=30,        TRUE, FALSE, FALSE),
    pf_Vf_lo  = if_else(Vf2 >= 1.2,        TRUE, FALSE, FALSE),
    pf_Vf_hi  = if_else(Vf2 <= 2.0,        TRUE, FALSE, FALSE),
    pf_Pf     = if_else(Pf2 >= 80,         TRUE, FALSE, FALSE),
    pf_KINK   = if_else(abs(KINK) <= 30,   TRUE, FALSE, FALSE),
    pf_SMSR   = if_else(SMSR2 >= 35,       TRUE, FALSE, FALSE),
    pf_final  = if_all(pf_Ith:pf_SMSR)
  )
...
df_summary_liv2 = df_summary_liv |> 
  mutate(
    .by = c(dut_id),
...
df_summary_liv2 |>
  select(dut_id, bi_status, contains("Ith1d"), contains("Pf2")) |> 
...
df_summary_liv2 |> 
  ggplot(aes(x = bi_status, y = dPf2_pct, group = dut_id)) +
  geom_line(alpha = 0.5) +
