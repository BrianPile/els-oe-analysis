...
df_yield_oe =   df_summary |> 
    ...
    mutate(
      pf_oe_final = pf_oe_spec & pf_oe_burn
    )
...
df_summary |> 
  ...
  arrange(bi_status) |> 
  mutate(
    result_code = fct_reorder(result_code, pct, .desc = TRUE),
    bar_color = if_else(result_code == "PASS SPEC", "green", "red")
  ) |> 
  ggplot(aes(x = result_code, y = pct, fill = bar_color)) +
